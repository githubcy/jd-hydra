            ...
            getMyTrace :function (trace, myScope) {
                ...
                function getMySpan(span, spanIndex, myParent, index) {
                    ...
                    for (var i in span.annotations) {
                        if (span.annotations[i]['value']=='client_send') {
                            anMap['client_send'] = span.annotations[i]['timestamp'];
                            continue;
                        }
                        if (span.annotations[i]['value']=='server_send') {
                            anMap['server_send'] = span.annotations[i]['timestamp'];
                            continue;
                        }
                        if (span.annotations[i]['value']=='server_receive') {
                            anMap['server_receive'] = span.annotations[i]['timestamp'];
                            ip = span.annotations[i]['host']['ip'];
                            continue;
                        }
                        if (span.annotations[i]['value']=='client_receive') {
                            anMap['client_receive'] = span.annotations[i]['timestamp'];
                            continue;
                        }
                    }
                    ...
                    if (myParent){//如果有父span
                        if (index == 0){//如果是第一个子span
                            //按照父span的时间处理
                            anMap['client_send'] = myParent.anMap['server_receive'];
                        }else {
                            //按照自己前一个兄弟span的时间处理
                            var myBrother = myParent.children[index - 1];
                            anMap['client_send'] = myBrother['client_receive'];
                        }
                    }
                    anMap['client_receive'] = parseInt(anMap['client_send']) + parseInt(span.durationClient)
                    //sr约等于网络消耗时长的一半
                    anMap['server_receive'] = parseInt(anMap['client_send']) + (parseInt(span.durationClient) - parseInt(span.durationServer)) / 2;



                    span.used = {
                        spanId: span.id,
                        start: parseInt(anMap['client_send']),
                    ...
                    span.wasted = {
                        spanId: span.id,
                        start: parseInt(anMap['client_send']) + parseInt(span.durationServer),
                        duration: function(){
...
//                if (!rootSpan.children) return;
                ...
                var enterTransition = enter.transition()
                    ...
                    .attr("transform", function (time) {
                        //return 'translate(' + view.x(time.duration - rootSpan.used.duration) + ',' + view.y * time.viewIndex * 1.2 + ')';
