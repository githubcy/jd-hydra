...
public class ServiceMapperTest extends AbstractDependencyInjectionSpringContextTests {
    ...
    public void testDataBaseOption()throws Exception{
        try {
            ...
            Assert.assertNotNull(queryPara);
            Assert.assertEquals(servicePara, queryPara);
            //modify entity
    ...
    public void testFindByName(){
        try {
            ...
            Assert.assertNotNull(servicePara1);
            Assert.assertNotNull(servicePara2);
            ...
            Assert.assertEquals(id1, s1.getId());
            Assert.assertEquals(id2, s2.getId());
            Assert.assertEquals(appId, s1.getAppId());
        }catch (Exception e){
           e.printStackTrace();
        }finally {
            //最后删除所有的测试数据
